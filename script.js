...
const PROVIDERS = {
  ...
  cerebras: {
    ...
    models: [
      { v:'qwen-3-235b-a22b-instruct-2507', l:'Qwen3 235B ⭐ — Mạnh nhất, tiếng Việt xuất sắc' },
      { v:'qwen3-32b',                       l:'Qwen3 32B — Nhanh hơn, reasoning realtime' },
      { v:'llama3.1-8b',                     l:'Llama 3.1 8B — Nhẹ nhất, nhanh nhất' },
      { v:'__custom__',                       l:'✏️ Tự nhập model...' },
    ]
  },
  ollama: {
    label: '🦙 Ollama', color: '#4caf89',
    keyUrl: 'https://ollama.com/download', keyLabel: 'Ollama Download',
    placeholder: '(không cần key)',
    badge: 'LOCAL', badgeColor: 'var(--accent2)',
    guide: `Chạy AI hoàn toàn trên máy bạn — miễn phí, riêng tư, không giới hạn.<br><br>
<b>Bước 1:</b> Tải & cài Ollama tại <a href="https://ollama.com/download" target="_blank" style="color:var(--accent2);">ollama.com/download</a><br>
<b>Bước 2:</b> Mở Terminal / CMD, chạy lệnh tải model:<br>
<code style="background:var(--bg);padding:2px 6px;border-radius:3px;font-size:11px;">ollama pull qwen2.5</code> &nbsp;hoặc&nbsp; <code style="background:var(--bg);padding:2px 6px;border-radius:3px;font-size:11px;">ollama pull llama3.2</code><br>
<b>Bước 3 (quan trọng):</b> Bật CORS để app có thể kết nối:<br>
&nbsp;&nbsp;• <b>Windows:</b> Tạo biến môi trường hệ thống <code style="background:var(--bg);padding:2px 6px;border-radius:3px;font-size:11px;">OLLAMA_ORIGINS=*</code> rồi restart Ollama<br>
&nbsp;&nbsp;• <b>Mac:</b> <code style="background:var(--bg);padding:2px 6px;border-radius:3px;font-size:11px;">launchctl setenv OLLAMA_ORIGINS "*"</code> rồi restart<br>
&nbsp;&nbsp;• <b>Linux:</b> Thêm <code style="background:var(--bg);padding:2px 6px;border-radius:3px;font-size:11px;">Environment="OLLAMA_ORIGINS=*"</code> vào ollama.service rồi reload<br>
<b>Bước 4:</b> Chọn model bên dưới, để trống ô key → bấm Lưu`,
    models: [
      { v:'qwen2.5:14b',  l:'Qwen2.5 14B ⭐ — Tiếng Việt tốt nhất local (12GB RAM)' },
      { v:'qwen2.5:7b',   l:'Qwen2.5 7B — Nhẹ hơn, tiếng Việt khá (8GB RAM)' },
      { v:'llama3.2:3b',  l:'Llama 3.2 3B — Siêu nhẹ, chạy mọi máy (4GB RAM)' },
      { v:'gemma3:12b',   l:'Gemma 3 12B — Google open-source (10GB RAM)' },
      { v:'mistral',      l:'Mistral 7B — Nhanh, tiếng Anh tốt (8GB RAM)' },
      { v:'__custom__',   l:'✏️ Tự nhập tên model đã pull...' },
    ]
  },
};
  ...
  if (flatList.length) {
    ...
    flatList.forEach(({ pKey, idx, val }) => {
      ...
      btn.onclick = () => {
        ...
        selectProvider(pKey, null, db2[currentUser].keys, db2[currentUser].activeKeyIdx);
...
async function apiTestAndSave(doSave) {
  ...
  try {
    ...
    if (_curProvider === 'gemini') {
      ...
      const h = {'Content-Type':'application/json','Authorization':`Bearer ${key||'ollama'}`};
...
function preparePromptAndStartStory() {
  ...
  const masterPrompt = `
    ...
    1. Cơ chế Mở: Hướng phát triển của nhân vật chính hoàn toàn do người dùng chọn.
    ...
    4. Lựa chọn hành động: Sau mỗi đoạn truyện, đưa ra đúng 3-4 lựa chọn hành động CHO NHÂN VẬT CHÍNH.
       - Mỗi lựa chọn là hành động của ${charName || 'nhân vật chính'}, bắt đầu bằng động từ chủ động.
       - Tối đa 50 từ mỗi lựa chọn. Ngắn gọn, rõ ràng.
       - TUYỆT ĐỐI KHÔNG: lựa chọn về hành động của nhân vật phụ, về bối cảnh.
    ...
    ***
    A. [Hành động ngắn của ${charName || 'nhân vật chính'} — tối đa 50 từ]
    B. [Hành động ngắn của ${charName || 'nhân vật chính'} — tối đa 50 từ]
    C. [Hành động ngắn của ${charName || 'nhân vật chính'} — tối đa 50 từ]
    D. [Hành động ngắn của ${charName || 'nhân vật chính'} — tối đa 50 từ] (tùy chọn, nếu phù hợp)
  `;  
...
function continueStory(id) {
  ...
  for(let i=0; i < currentStoryData.history.length; i++) {
     ...
     if(msg.role === 'user') {
         ...
         if (i === 1) {
           if (parts.length >= 4) {
             storyText = parts[1] + "\n\n" + parts[2]; choiceText = parts[3];
           } else if (parts.length >= 3) {
             storyText = parts[1]; choiceText = parts[2];
           } else { storyText = parts[0]; choiceText = ""; }
         } else {
           storyText = parts[0]; choiceText = parts.length >= 2 ? parts[1] : "";
         }
         chatBox.innerHTML += `<div class="msg ai">${storyText.trim().replace(/\n/g, '<br>')}</div>`;
         if (i === currentStoryData.history.length - 1) renderChoices(choiceText);
     }
  }
  ...
  try {
    ...
    if (provider === 'gemini') {
      ...
      if (!d.choices || !d.choices[0] || !d.choices[0].message) {
        throw new Error('Response không hợp lệ: ' + JSON.stringify(d).slice(0, 200));
      }
      // Strip <think>...</think> của Qwen3 thinking mode (Groq, Cerebras)
      aiResponseText = d.choices[0].message.content.replace(/<think>[\s\S]*?<\/think>/gi, '').trim();
    }

    // Xử lý response — lần đầu có 4 phần: [tên]***[giới thiệu]***[chương 1]***[lựa chọn]
    const parts = aiResponseText.split("***");
    let storyText = "", choiceText = "";
    if (currentStoryData.history.length===1 && parts.length>=4) {
      // Format mới: tên *** giới thiệu *** chương 1 *** lựa chọn
      currentStoryData.title = parts[0].trim() || "Chuyến Phiêu Lưu Vô Danh";
      document.getElementById('current-story-title').innerText = currentStoryData.title;
      storyText = parts[1] + "\n\n" + parts[2]; // giới thiệu + chương 1
      choiceText = parts[3];
    } else if (currentStoryData.history.length===1 && parts.length>=3) {
      // Format cũ: tên *** nội dung *** lựa chọn
      currentStoryData.title = parts[0].trim() || "Chuyến Phiêu Lưu Vô Danh";
      document.getElementById('current-story-title').innerText = currentStoryData.title;
      storyText = parts[1]; choiceText = parts[2];
